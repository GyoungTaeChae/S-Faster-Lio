...
/*
IMU data preprocessing: IMU initialization, IMU forward propagation,
backward propagation to compensate motion distortion
*/
...
// Backward propagation: undistort point cloud
void ImuProcess::UndistortPcl(const MeasureGroup& meas,
  ...
  for (auto it_kp = IMUpose.end() - 1; it_kp != IMUpose.begin(); it_kp--) {
    ...
    for (; it_pcl->curvature / double(1000) > head->offset_time; it_pcl--) {
      ...
      M3D R_i(R_imu *
              Sophus::SO3<double>::exp(angvel_avr * dt).matrix());
