...
LiDAR 점 p ──→ 월드 좌표로 변환 ──→ 맵의 평면과 비교
            ...
            거리 d = nᵀ·(p_world - pₘ)
                   │
                   ▼
         이상적으로 d = 0 (점이 평면 위에 있어야 함)
```

1. **구조화된 환경**: 실내/도시 환경에서 평면이 많음 (벽, 바닥, 천장)
2. **안정성**: 점-점 매칭보다 노이즈에 강함 (여러 점으로 평면 피팅)
3. **계산 효율**: 거리 계산이 내적 한 번 (`nᵀ·Δp`)
4. **야코비 단순**: 미분이 간단하여 최적화에 유리

**측정 함수:**

```
z = h(x) = nᵀ·(R·(R_L·p + T_L) + t - pₘ)
```
